        ...
        true ->
            TcpMod:send(Sock,
                efirebirdsql_op:op_create(
                    Username, Password, Database, PageSize, State));
        false ->
            TcpMod:send(Sock,
...
connect(Host, Username, Password, Database, IsCreateDB, PageSize, State) ->
    ...
    TcpMod:send(Sock,
        efirebirdsql_op:op_connect(Host, Username, Password, Database, State)),
    case efirebirdsql_op:get_connect_response(TcpMod, Sock, State) of
        {ok, NewState} ->
            connect_database(TcpMod, Sock, Username, Password, Database, PageSize, IsCreateDB, NewState);
...
connect(Host, Username, Password, Database, Options, State) ->
    ...
    case (State#state.mod):connect(Host, Port, SockOptions) of
        {ok, Sock} ->
            ...
            connect(Host, Username, Password, Database, IsCreateDB, PageSize, State2);
        {error, Reason} ->
            {error, Reason, State}
    end.
